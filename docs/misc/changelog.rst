...
- On-policy algorithms (PPO and A2C) now use the logging key `time/total timesteps` instead of `time/total_timesteps`. This matches the behavior of the other algorithms and the eval callback. (@timokau)
